                ...
                <div class="d-inline-block align-items-center">
                    <nav>
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active alignment-text-new" aria-current="page"><!-- Reminders --></li>
                        </ol>
                    </nav>
                </div>
            </div>
                        ...
                        <div class="card-actions float-end">
                            <div class="dropdown show">
                                <!-- <a href="{{ route('reminder.add') }}"
                                    class="waves-effect waves-light btn btn-sm btn-outline btn-info mb-5">Add Reminder</a> -->
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                                ...
                                <tbody class="text-dark">
                                    @if($datas->isNotEmpty())
                                        @foreach ($datas as $key => $data)
                                                <tr onclick="window.location='{{ route('reminder.detail', ['id' => $data->id, 'student_reminder_noty' => $data->student_reminder_noty]) }}';" 
                                                style="cursor: pointer; background-color: {{ $data->student_reminder_noty == 'student' ? '#f2f2f2' : 'transparent' }};">
                                                    <td>
                                                    @if($data->student_reminder_noty == 'student')
                                                            <b>{{ $key + 1 }}</b>
                                                        @else
                                                            {{ $key + 1 }}
                                                        @endif
...
$(document).on('click', '#verify_admin_password', function() {
            ...
            $.ajax({
                url: "{{ route('reminder-verify-admin') }}",
                type: "POST",
                data: {
