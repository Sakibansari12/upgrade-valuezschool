                ...
                <div class="d-inline-block align-items-center">
                    <nav>
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active alignment-text-new" aria-current="page">Manage What's New</li>
                        </ol>
                    </nav>
                </div>
            </div>
                                ...
                                <tbody class="text-dark">
                                    @if($datas->isNotEmpty())
                                        @foreach ($datas as $key => $data)
                                                <tr onclick="window.location='{{ route('notify.detail', ['id' => $data->id, 'student_noty' => $data->student_noty]) }}';" 
                                                style="cursor: pointer; background-color: {{ $data->student_noty == 'student' ? '#f2f2f2' : 'transparent' }};">
                                                    <td>
                                                    @if($data->student_noty == 'student')
                                                            <b>{{ $key + 1 }}</b>
                                                        @else
                                                            {{ $key + 1 }}
                                                        @endif
