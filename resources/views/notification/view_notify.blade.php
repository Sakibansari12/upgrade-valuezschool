                                ...
                                <tbody class="text-dark">
                                    @if($datas->isNotEmpty())
                                        @foreach ($datas as $key => $data)
                                                <tr onclick="window.location='{{ route('notify.detail', ['id' => $data->id, 'school_admin_noty' => $data->school_admin_noty]) }}';" 
                                                style="cursor: pointer; background-color: {{ $data->school_admin_noty == 'admin' ? '#f2f2f2' : 'transparent' }};">
                                                    <td>
                                                    @if($data->school_admin_noty == 'admin')
                                                            <b>{{ $key + 1 }}</b>
                                                        @else
                                                            {{ $key + 1 }}
                                                        @endif
