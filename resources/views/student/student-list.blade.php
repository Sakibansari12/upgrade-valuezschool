    ...
    <section class="content">
        <div class="row">
            <div class="col-xl-12">
                ...
                <div class="card">
                    ...
                    <form action="{{ route('student.list') }}" method="GET">
                        <div class="row" >
                            ...
                            <div class="col-md-8">
                                <div class="form-group">
                                    <select name="student_status" id="student_status_filter" onchange="this.form.submit()" class="form-control filter-button">
                                        <option value="">Select Student Status</option>
                                        <option value="demo" @if($student_status == 'demo') selected @endif>Demo</option>
                                        <option value="paid" @if($student_status == 'paid') selected @endif>Paid</option>
                                        <option value="pending" @if($student_status == 'pending') selected @endif>Pending</option>
                                    </select>
                                </div>
                            </div>
                            </div>
                           <!--  <input type="hidden" name="school_id" onchange="this.form.submit()" id="school_id" value="{{$schoolid}}">
                            <input type="hidden" name="convert_select[]" onchange="this.form.submit()"  id="convert_select"> -->
                 <!-- <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                    ...
                    <div class=" col-md-4">
                        ...
                        <label class="form-label">Change Status</label>
                            <select name="student_update_status" id="student_update_status_filter" onchange="StatusUpdate(); this.form.submit();"  class="form-control filter-button">
                ...
                @if(!empty($schooldata) && !empty($schooldata->access_code == 'vz1234'))
                        <div class="row">
            ...
            <div class="container">
                <div class="row">
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            <label class="form-label">Change Status</label>
                            <select name="student_update_status" id="student_update_status_filter"  class="form-control filter-button">
                                <option value="">Select</option>
                                <option value="demo">Demo</option>
                                <option value="paid">Paid</option>
                                <option value="pending">Pending</option>
                    ...
                    <input type="hidden" name="convert_select[]" onchange="this.form.submit()"  id="convert_select">
                    <div class="col-md-1 ">
                        <button type="submit" onclick="StatusUpdate()" class="btn btn-sm" style="background-color: #00205c; color: #fff; margin-top: 28px;">Update</button>
                    </div>
                </div>
            </div>
        </form>
                                    ...
                                    @foreach ($student as $data)
                                        ...
                                        <td class="simple-table-td" style="max-width: 300px;">
                                            ...
                                            <p class="m-0 custom-nowrap-ellipsis" id="copyContainer_{{$data->id}}">
                                                ...
                                                <p class="m-0 custom-nowrap-ellipsis"><b>Start Date: </b>
                                                {{ isset($data->start_date) ? \Carbon\Carbon::parse($data->start_date)->format('d/m/Y') : '' }}
                                            </p>
                                                <p class="m-0 custom-nowrap-ellipsis"><b>End Date: </b>
                                                {{ isset($data->end_date) ? \Carbon\Carbon::parse($data->end_date)->format('d/m/Y') : '' }}
                                            </p>
                                                <!-- <p class="m-0 custom-nowrap-ellipsis"><b>Password: </b>{{ $data->view_password }}</p> -->
                                            </div>
                                           </td>

                                           
                                            <td>
                                              <button class="btn btn-sm btn-outline-secondary copy-button" style="margin-left: 50px; background-color: #00205c; color: #fff;" onclick="copyToClipboard({{ $data->id }})">Copy
                                              </button>
                                            </td>
                                             ...
                                             <td>
                                                <!-- @if($data->section)
                                                    {{ substr($data->section, 0, 15) }}
                                                    <a href="javascript:void(0);" data-description="{{ $data->section }}" class="section_popup">....</a>
                                                @endif -->
                                                @if($data->section)
                                                    @php
                                                        $section = $data->section;
                                                        $displayedSection = strlen($section) > 15 ? substr($section, 0, 15) . '...' : $section;
                                                    @endphp
                                                    {{ $displayedSection }}
                                                    @if(strlen($section) > 15)
                                                        <a href="javascript:void(0);" data-description="{{ $section }}" class="section_popup">....</a>
                                                    @endif
                                                @endif
                                            ...
                                            @if(($user->usertype == 'superadmin'))
                                                ...
                                                <a href="{{ route('student.logs.list', ['userid' => $data->user_id]) }}"
  ...
  $('#section-popup-model').modal('show');
    ...
    function StatusUpdate(){
        ...
        if (selectedCheckboxes.length === 0) {
                alert("Please check at least one checkbox before update.");
                return false;
        }else{
