...
@section('content')
                    ...
                    <nav>
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item" aria-current="page"><a
                                    href="{{ route('teacher.class.list') }}">{{ $gradedata->class_name }}</a></li>
                                    <li class="breadcrumb-item" aria-current="page"><a
                                    href="{{ route('teacher.course.list', ['class' => $gradedata->id]) }}">Course</a></li>
                            <li class="breadcrumb-item active" aria-current="page">AI Module</li>
                        </ol>
                    </nav>
                </div>
            </div>
    ...
    <section class="content">
        <div class="row">
            <div class="col-12">
                <div class="row">
                    @if (!empty($aimodules))
                        @foreach ($aimodules as $cdata)
                            @php
                                if (($cdata->aimodule_status == 1 && $check_premium->is_demo == 1) || $check_premium->is_demo == 0) {
                                    $is_demo_content = 1;
                            ...
                            <div class="col-xl-4 col-md-6 col-12 px-4">
                                ...
                                @endif
                                    ...
                                    <a href="{{ $is_demo_content == '#' ? '#' : route('chat-gpt', 
                                        ['id' => $cdata->id, 'type' => $cdata->type, 'course_id' => $coursesdata->id, 'grade_id' => $gradedata->id]) }}"
                                        
                                        >
                                    <img class="video-btn"
                                        src="{{ url('uploads/Identifier') }}/{{ $cdata->thumbnail ? $cdata->thumbnail : 'no_image.png' }}"
                                        alt="{{ $cdata->display_name }}" 
                                        data-title="{{ $cdata->display_name }}" 
                                        data-bs-target="{{ $is_demo_content == '#' ? '#bs-video-modal-demo' : '#' }}"
                                        data-bs-toggle="{{ $is_demo_content == '#' ? 'modal' : '#' }}"
                                        data-bs-toggle="modal" />
                                    </a>



                                <!-- <a href="{{ route('chat-gpt', ['id' => $cdata->id, 'type' => $cdata->type, 'course_id' => $coursesdata->id,'grade_id' => $gradedata->id]) }}">
                                    <img class="video-btn"
                                        src="{{ url('uploads/aimodule') }}/{{ $cdata->thumbnail ? $cdata->thumbnail : 'no_image.png' }}"
                                        alt="{{ $cdata->display_name }}" 
                                        data-title="{{ $cdata->display_name }}" 
                                        data-bs-toggle="modal" />
                                </a> -->
                                    <div class="card-body">
                                        <h5 class="card-title">{{ $cdata->display_name }}</h5>
                                        <div class="mb-10">
                                            <ul class="list-unstyled d-flex justify-content-between align-items-center">
                                                <li><a href="#" class="text-mute" ><i
                                                            class="fa fa-tag"></i>
                                                            {{ $gradedata->class_name }}</a></li>
                                                <li><a href="#" class="text-mute"><i
                                                            class="fa fa-folder-open-o"></i>
                                                        {{ $coursesdata->course_name }}</a></li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
    ...
    <div class="modal fade" id="bs-video-modal-demo" tabindex="-1" role="dialog" aria-labelledby="modal-label-demo"
        ...
        <div class="modal-dialog modal-dialog-centered modal-width-increse">
            <div class="modal-content">
                <div class="modal-header">
